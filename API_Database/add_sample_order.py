"""
Script to add a sample order for testing
"""
...
def add_sample_order():
    ...
    try:
        ...
        cursor.execute("SELECT menu_id, item_name, price FROM menu WHERE outlet_id = %s LIMIT 3", (outlet_id,))
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s)
        """, (member_id, outlet_id, total_amount, 'Pending', datetime.now()))
        ...
        for menu_id, item_name, quantity, subtotal in order_items:
            ...
            logging.info(f"Added order item: {item_name}")
            
        # Commit the transaction
        conn.commit()
        logging.info(f"Sample order created successfully with total amount: ₹{total_amount}")
