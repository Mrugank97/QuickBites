...
def register_routes(app):
    ...
    # API endpoint to get outlet information for a specific manager
    @app.route('/api/outlet/manager/<member_id>', methods=['GET'])
    @require_valid_session
    def get_outlet_for_manager_by_id(member_id, user_id=None, **kwargs):
        try:
            # Check if user is authorized (admin, the manager themselves, or another manager)
            user_role = kwargs.get('user_role', '').lower()
            if not (user_role == 'admin' or 'manager' in user_role or str(user_id) == str(member_id)):
                return jsonify({"error": "Unauthorized. Only admins and managers can view outlet assignments"}), 403

            # Handle 'undefined' member_id
            if member_id == 'undefined':
                if 'manager' in user_role:
                    # If the user is a manager, return their own outlet
                    member_id = user_id
                else:
            ...
            # Get outlet information for the manager
            query = """
            SELECT om.outlet_id, o.name, o.location
            ...
            logging.error(f"Error in get_outlet_for_manager_by_id: {str(e)}")
